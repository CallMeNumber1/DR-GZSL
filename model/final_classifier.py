...
class CLASSIFIER:
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):
                ...
                inputv = self.input
                labelv = self.label # 离散的
                # print(labelv.unique())

                output = self.model(inputv)
                '''
                    TODO ZSL设置下，执行到这一句会报错
                    不过目前也不需要跑ZSL下的情况，因此先忽略这个
                '''
    ...
    def compute_per_class_acc_gzsl(self, test_label, predicted_label, target_classes):
        ...
        for i in range(target_classes.size()[0]):
            '''
                is_class为预测为目标类i的位置
                因为predicted_label是测试集全部样本的预测结果，因此is_class不会为0，那么下面计算时就不会出现除0错误。
            '''
